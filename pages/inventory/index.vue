<template>
    <v-container class="my-10">
        <v-row justify="space-between">
          <v-col><h2>Products (5)</h2></v-col>
          <v-col class="text-right"><v-btn rounded size="large" variant="tonal" to="/inventory/new" prepend-icon="mdi-package-variant-plus" color="primary">New Product</v-btn></v-col>
        </v-row>
        ...
        <div class="my-10">
          ...
          <v-data-table-server :items-length="totalCustomers" :search="searchKey" :items="customers" @update:options="updateTable">
            <template v-slot:item.action="item">
              <v-btn variant="outlined" color="grey-darken-2" :to="`/inventory/${item.item.id}`" prepend-icon="mdi-eye">View</v-btn>
            </template>
          </v-data-table-server>
        </div>
    </v-container>
...
const customers = ref([
  {
    "Serial Id": "10000298734724",
    "brand": "Apple",
    "model": "iPhone",
    "unit": "15 Pro Max 256GB White",
    "cod": 70421.48,
    "action": ""
  },
])
